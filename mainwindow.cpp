...
//--------------------------------------------------SouthAntennaDescriptor-------------------------------------------------------------------
    ...
    for (unsigned int i = 0;i < southAntennaNumber;++i){
        settings.setArrayIndex(i);
        QStringList n_r_c_x_y_z_fE_feed_D_T = settings.value("SouthAntennaDescriptor","S2001, 0, 0, 0, 0, 0, 0, 0, 0, 0").toString().split(",");
...
//--------------------------------------------------EastAntennaDescriptor-------------------------------------------------------------------
    ...
    for (unsigned int i = 0;i < eastAntennaNumber;++i){
        settings.setArrayIndex(i);
        QStringList n_r_c_x_y_z_fE_feed_D_T = settings.value("EastAntennaDescriptor","E2001, 0, 0, 0, 0, 0, 0, 0, 0, 0").toString().split(",");
...
//--------------------------------------------------WestAntennaDescriptor-------------------------------------------------------------------
    ...
    for (unsigned int i = 0;i < westAntennaNumber;++i){
        settings.setArrayIndex(i);
        QStringList n_r_c_x_y_z_fE_feed_D_T = settings.value("WestAntennaDescriptor","W2001, 0, 0, 0, 0, 0, 0, 0, 0, 0").toString().split(",");
...
//--------------------------------------------------EastWestAntennaDescriptor---------------------------------------------------------------
    eastWestAntennaNumber = westAntennaNumber + eastAntennaNumber;
    ...
    for (;i < eastWestAntennaNumber;++i){
        pEastWestAntennaReceiver[i] = pEastAntennaReceiver[i - westAntennaNumber];
        pEastWestAntennaChannel[i] = pEastAntennaChannel[i - westAntennaNumber];
        pEastWestAntennaName[i] = pEastAntennaName[i - westAntennaNumber];
    }
//--------------------------------------------------visIndexCalculation---------------------------------------------------------------
    southEastWestVisNumber = southAntennaNumber * (eastAntennaNumber + westAntennaNumber);
...
//southWest vis
        for (unsigned j = 0;j < westAntennaNumber;++j){
            ...
            unsigned int visInd = i * (eastAntennaNumber + westAntennaNumber) + j;
            ...
            pVis0612AntennaNameA[visInd] = pSouthAntennaName[i];
            pVis0612AntennaNameB[visInd] = pWestAntennaName[j];
        }
//southEast vis
        for (unsigned j = 0;j < eastAntennaNumber;++j){
            unsigned int eastAntennaIndex = pEastAntennaName[j].right(2).toUInt() - 1;
            unsigned int visInd = i * (eastAntennaNumber + westAntennaNumber) + j + westAntennaNumber;
            ...
            pVisibilitySign[visInd] = H > V ? 1 : -1;
            pVis0612AntennaNameA[visInd] = pSouthAntennaName[i];
            pVis0612AntennaNameB[visInd] = pEastAntennaName[j];
        }
    }
//southSouth vis
    for (unsigned int i = 1, visInd = southEastWestVisNumber;i < southAntennaNumber;++i){
        for (unsigned int j = 0;j < southAntennaNumber - i;++j){
            ...
            pVisIndToCorrPacketInd[visInd] = visibilityIndexAsHV(H, V);
            pVisibilitySign[visInd] = H > V ? 1 : -1;
            pVis0612AntennaNameA[visInd] = pSouthAntennaName[j];
            pVis0612AntennaNameB[visInd++] = pSouthAntennaName[j+i];
        }
    }
    for (unsigned int i = 1, visInd = southEastWestVisNumber + southVisNumber;i < eastWestAntennaNumber;++i){
        for (unsigned int j = 0;j < eastWestAntennaNumber - i;++j){
            ...
            if (j < westAntennaNumber && (j + i) < westAntennaNumber)
                ...
                pVisibilitySign[visInd] = 1;
            pVis0612AntennaNameA[visInd] = pEastWestAntennaName[j];
            pVis0612AntennaNameB[visInd++] = pEastWestAntennaName[j+i];
        }
    }
//--------------------------------------------------ampIndexCalculation---------------------------------------------------------------
    ...
    pAntennaName = new QString[amp0612Number];
    pAntennaReceiver = new unsigned int[amp0612Number];
    pAntennaReceiverChannel = new unsigned int[amp0612Number];
    ...
    for (;ant < westAntennaNumber + eastAntennaNumber;++ant){
        pAmpIndToFitsInd[ant] = ant;
        pAntennaReceiver[ant] = pEastAntennaReceiver[ant - westAntennaNumber];
        pAntennaReceiverChannel[ant] = pEastAntennaChannel[ant - westAntennaNumber];
        pAmpIndToCorrPacketInd[ant] = pEastAntennaReceiver[ant - westAntennaNumber]*16 + pEastAntennaChannel[ant - westAntennaNumber];
        pAntennaName[ant] = pEastAntennaName[ant - westAntennaNumber];
        pAntennaDelay[ant] = pEastAntennaDelay[ant - westAntennaNumber];
    }
    for (;ant < southAntennaNumber + westAntennaNumber + eastAntennaNumber;++ant){
        pAmpIndToFitsInd[ant] = ant;
        pAntennaReceiver[ant] = pSouthAntennaReceiver[ant - eastAntennaNumber - westAntennaNumber];
        pAntennaReceiverChannel[ant] = pSouthAntennaChannel[ant - eastAntennaNumber - westAntennaNumber];
        pAmpIndToCorrPacketInd[ant] = pSouthAntennaReceiver[ant - eastAntennaNumber - westAntennaNumber]*16 + pSouthAntennaChannel[ant - eastAntennaNumber - westAntennaNumber];
        pAntennaName[ant] = pSouthAntennaName[ant - eastAntennaNumber - westAntennaNumber];
        pAntennaDelay[ant] = pSouthAntennaDelay[ant - eastAntennaNumber - westAntennaNumber];
    }
//--------------------------------------------------rawDataAndFITS---------------------------------------------------------------
    numberOfVisibilities = 22256;
    numberOfFitsVisibilities = vis0612Number;
    numberOfFitsAmplitudes = amp0612Number;
    ...
    antNameIndexColumn.resize(amp0612Number);
    antFrontEndColumn.resize(amp0612Number);
    antFeedColumn.resize(amp0612Number);
    antDiameterColumn.resize(amp0612Number);
    antXColumn.resize(amp0612Number);
    antYColumn.resize(amp0612Number);
    antZColumn.resize(amp0612Number);
    ...
    for (unsigned int i = 0;i < amp0612Number;++i){
        if (i < westAntennaNumber){
            ...
            antIndexColumn[i] = i;
        } else if (i < eastAntennaNumber + westAntennaNumber){
            antFrontEndColumn[i] = pEastAntennaFrontEndID[i - westAntennaNumber];
            antFeedColumn[i] = pEastAntennaFeedID[i - westAntennaNumber];
            antDiameterColumn[i] = pEastAntennaDiameter[i - westAntennaNumber];
            antXColumn[i] = pEastAntennaX[i - westAntennaNumber];
            antYColumn[i] = pEastAntennaY[i - westAntennaNumber];
            antZColumn[i] = pEastAntennaZ[i - westAntennaNumber];
            antIndexColumn[i] = i;
        } else if (i < southAntennaNumber + westAntennaNumber + eastAntennaNumber){
            antFrontEndColumn[i] = pSouthAntennaFeedID[i - eastAntennaNumber - westAntennaNumber];
            antFeedColumn[i] = pSouthAntennaFrontEndID[i - eastAntennaNumber - westAntennaNumber];
            antDiameterColumn[i] = pSouthAntennaDiameter[i - eastAntennaNumber - westAntennaNumber];
            antXColumn[i] = pSouthAntennaX[i - eastAntennaNumber - westAntennaNumber];
            antYColumn[i] = pSouthAntennaY[i - eastAntennaNumber - westAntennaNumber];
            antZColumn[i] = pSouthAntennaZ[i - eastAntennaNumber - westAntennaNumber];
            antIndexColumn[i] = i;
        }
...
*/
    for (ant = 0;ant < amp0612Number;++ant)
        antName2indexMap[pAntennaName[ant]] = antIndexColumn[ant];

    antAColumn.resize(vis0612Number);
    antBColumn.resize(vis0612Number);
    for (unsigned int vis = 0;vis < vis0612Number;++vis){
        antAColumn[vis] = antName2indexMap[pVis0612AntennaNameA[vis]];
        antBColumn[vis] = antName2indexMap[pVis0612AntennaNameB[vis]];
    }
//--------------------------------------------------fileLog---------------------------------------------------------------
	...
	out << soldat.DDeclination() << "\n";
        ...
        for (unsigned int i = 0;i < vis0612Number;++i)
                out << i << " "
                    ...
                    << antName2indexMap[pVis0612AntennaNameA[i]] << ")\t"
                    << pVis0612AntennaNameB[i] << "\t("
                    << antName2indexMap[pVis0612AntennaNameB[i]] << ")\t"
                    << pVisIndToFitsInd[i] << "\t"
...
//--------------------------------------------------UI---------------------------------------------------------------
    ...
    ui->antennaPlotter->xAxis->setRange(0, amp0612Number);
...
void MainWindow::writeCurrentFits(){
    ...
    QString qFitsName = "currentData/SRH612/" + QDateTime::currentDateTime().toString("yyyyMMdd") + "/srh_0612_" + QDateTime::currentDateTime().toString("yyyyMMddThhmmss") + ".fit";
...
//------
    ...
    addKey2FitsHeader(QString("INSTRUME"),QString("SRH0612"),QString("3-6 GHz Siberian Radioheliograph"),&fits2save);
    ...
    try{
        Table* pAntNameTable = fits2save.addTable(antNameHduSign, amp0612Number, antNameColName, antNameColForm, antNameColUnit, AsciiTbl);
    ...
    try{
        Table* pAntTable = fits2save.addTable(antHduSign, amp0612Number, antColName, antColForm, antColUnit);
    ...
    try{
        Table* pAntPairTable = fits2save.addTable(antPairHduSign, vis0612Number, antPairColName, antPairColForm, antPairColUnit);
...
void MainWindow::initDigitalReceivers(){
    ...
    setRgPacket.D.Rg32.Rg[1] = 0xFFF; //receiver selected
...
void MainWindow::setBitWindowPosition(unsigned int bitWindowPosition){
    ...
    setTypicalPacketPrefix(&setRgPacket, eRqst_Rdr_SetRgs32);
    setRgPacket.D.Rg32.Rg[0] = 0x0F; //receiver chip select address
    setRgPacket.D.Rg32.Rg[1] = 0xFFF; //1-12 receivers selected
    setRgPacket.D.Rg32.Rg[2] = 0x08; //internal receiver address (0x08 FIFO)
...
void MainWindow::initCorrelator(bool acquire){
    ...
    if(acquire){
        ...
        if (!QDir("currentData/SRH612/" + QDateTime::currentDateTime().toString("yyyyMMdd")).exists())
            QDir().mkdir("currentData/SRH612/" + QDateTime::currentDateTime().toString("yyyyMMdd"));
...
void MainWindow::initAntennaArrayLayoutPlotter(){
    ui->antennaArrayLayoutPlotter->xAxis->setRange(-350, 350);
    ...
    QVector<double> antennaX(amp0612Number);
    QVector<double> antennaY(amp0612Number);
    unsigned int i;
    ...
    for (i = 0;i < southAntennaNumber;++i){
        ...
        antennaX[i + southAntennaNumber + westAntennaNumber] = pEastAntennaY[i] * 0.001;
        antennaY[i + southAntennaNumber + westAntennaNumber] = pEastAntennaX[i] * 0.001;
    }
    ui->antennaArrayLayoutPlotter->graph(0)->setData(antennaX,antennaY,amp0612Number);
}
